...
def _format_views(min_views: int, max_views: int) -> str:
    return f"[{int(min_views)},{int(max_views)}]"


def _init_dataset_quietly(dataset_cls, *args, **kwargs):
    kwargs = dict(kwargs)
...
def _print_dataset_mix_table(rows: Sequence[DatasetLogRow]) -> None:
    if not rows:
        return
    headers = ("split", "dataset", "samples", "views", "img", "repeat", "extra")
    table = [
        (
            row.split,
            row.name,
...
def parse_weight_map(value: Optional[str]) -> Optional[Dict[str, float]]:
    if value is None or value.strip() == "":
        return None
    out: Dict[str, float] = {}
    for item in value.split(","):
        ...
        if "=" not in item:
            raise argparse.ArgumentTypeError(
                f"weight entry {item} is missing = separator (expected e.g. dl3dv=4)"
            )
        key, val = item.split("=", 1)
        key = key.strip().lower()
        try:
            ...
            raise argparse.ArgumentTypeError(f"weight entry {item} has non-numeric value") from exc
        if weight < 0:
            raise argparse.ArgumentTypeError(f"weight entry {item} must be non-negative")
        out[key] = weight
    return out or None


def join_oss_uri(root: str, relative: str) -> str:
...
def _oss_uri_to_rclone_path(oss_uri: str, remote: str) -> str:
    ...
    oss_path = uri[len("oss://"):]
    return f"{remote_name}:{oss_path}"


def _parse_rclone_lsd_dir_name(raw_line: str) -> Optional[str]:
    token = raw_line.strip()
    if not token:
        return None
    return token.rsplit(None, 1)[-1].strip("/")


def discover_trajectory_dirs_with_rclone(
    roots: Sequence[str],
    ...
    for root, oss_uri_root in zip(roots, oss_uri_roots):
        remote_path = _oss_uri_to_rclone_path(oss_uri_root, rclone_remote)
        command = _rclone_command(rclone_bin, rclone_config, "lsd", remote_path)
        result = subprocess.run(command, check=True, capture_output=True, text=True)
        root_path = Path(root)
        for line in result.stdout.splitlines():
            name = _parse_rclone_lsd_dir_name(line)
...
def discover_trajectory_dirs(
    ...
    if oss_uri_roots:
        try:
            ...
            print(f"[data][warn] ossutil trajectory scan failed ({oss_exc}); falling back to mounted path scan")
...
def read_manip_long6_camera_arrays(extrinsics_path: Path) -> Tuple[np.ndarray, Dict[int, np.ndarray]]:
    camera_dir = extrinsics_path.parent
    intrinsics_path = camera_dir / "intrinsics.npy"
    intrinsics = np.load(intrinsics_path).astype(np.float32)
    camera_to_env = np.load(extrinsics_path).astype(np.float32)
    if intrinsics.shape != (3, 3):
        raise ValueError(f"Expected 3x3 intrinsics in {intrinsics_path}, got {intrinsics.shape}")
    if camera_to_env.ndim != 3 or camera_to_env.shape[1:] != (4, 4):
        raise ValueError(f"Expected Nx4x4 camera-to-env extrinsics in {extrinsics_path}, got {camera_to_env.shape}")
    ...
    for frame_id, genmanip_camera_to_env in enumerate(camera_to_env):
        opencv_camera_to_env = genmanip_camera_to_env @ opencv_to_genmanip
        extrinsics_by_frame[frame_id] = np.linalg.inv(opencv_camera_to_env).astype(np.float32)
    return intrinsics, extrinsics_by_frame
...
class ManipTrajectoryDataset(Dataset):
    def __init__(
        ...
        w_stride_min: Optional[int] = None,
        w_stride_max: Optional[int] = None,
        ...
        # Manip_long6: short single-task trajectories with two wrist cams,
        # one fixed third-person cam, and one moving third-person cam.
        moving_stride_min: int = 2,
        moving_stride_max: int = 6,
        fixed_stride_min: int = 4,
        fixed_stride_max: int = 16,
        long6_root_marker: str = "Manip_long6",
        long6_mode_weights: Optional[Dict[str, float]] = None,
        moving_camera_prefix: str = "surround_cam_moving",
        fixed_camera_prefix: str = "surround_cam_fixed",
        ...
        self.long6_root_marker = long6_root_marker or "Manip_long6"
        invalid_scenes = [scene for scene in self.scene_dirs if self.long6_root_marker.lower() not in str(scene).lower()]
        if invalid_scenes:
            bad = ", ".join(str(scene) for scene in invalid_scenes[:3])
            raise ValueError(
                f"ManipTrajectoryDataset only supports Manip_long6 scenes; "
                f"paths must contain {self.long6_root_marker!r}. Invalid: {bad}"
            )
        ...
        self.w_stride_min = max(1, w_stride_min if w_stride_min is not None else self.random_stride_min)
        self.w_stride_max = max(self.w_stride_min, w_stride_max if w_stride_max is not None else self.random_stride_max)
        ...
        self.moving_stride_min = max(1, moving_stride_min)
        self.moving_stride_max = max(self.moving_stride_min, moving_stride_max)
        self.fixed_stride_min = max(1, fixed_stride_min)
        self.fixed_stride_max = max(self.fixed_stride_min, fixed_stride_max)
        default_long6 = {"W": 0.40, "T": 0.45, "F": 0.15}
        self.long6_mode_weights = self._normalize_long6_mode_weights(long6_mode_weights or default_long6)
        self.moving_camera_prefix = moving_camera_prefix
        self.fixed_camera_prefix = fixed_camera_prefix
        ...
        roots = [Path(root).expanduser() for root in (data_roots or [])]
        uris = list(oss_uri_roots or [])
        self._oss_uri_roots: List[Tuple[Path, str]] = list(zip(roots, uris))
    ...
    @staticmethod
    def _normalize_long6_mode_weights(weights: Dict[str, float]) -> Dict[str, float]:
        cleaned = {key: max(0.0, float(value)) for key, value in weights.items() if key in {"W", "T", "F"}}
        total = sum(cleaned.values())
        if total <= 0:
            return {"W": 0.40, "T": 0.45, "F": 0.15}
    ...
    def _oss_uri_for_scene(self, scene_dir: Path) -> Optional[str]:
        for root, uri_root in self._oss_uri_roots:
            try:
                rel = scene_dir.relative_to(root)
            except ValueError:
                continue
            return join_oss_uri(uri_root, rel.as_posix())
        return None

    def _stage_root_for_manip(self) -> Path:
        assert self.oss_stage_cache is not None
        return self.oss_stage_cache.stage_root / "Manip_long6" / "data"

    def _camera_names_for_long6_mode(self, mode: str) -> List[str]:
        if mode == "W":
            names = [name for name in MANIP_CAMERA_NAMES if name.startswith(self.wrist_camera_prefix)]
        elif mode == "T":
            names = [name for name in MANIP_CAMERA_NAMES if name.startswith(self.moving_camera_prefix)]
        elif mode == "F":
            names = [name for name in MANIP_CAMERA_NAMES if name.startswith(self.fixed_camera_prefix)]
        else:
            names = []
        return names or list(MANIP_CAMERA_NAMES)

    def _choose_long6_fallback_camera(self) -> Tuple[str, str]:
        enabled_modes = [
            mode for mode in ("W", "T", "F")
            if self.long6_mode_weights.get(mode, 0.0) > 0.0
        ] or ["W", "T", "F"]
        mode = self._weighted_mode_order(enabled_modes, self.long6_mode_weights)[0]
        camera_name = random.choice(self._camera_names_for_long6_mode(mode))
        return mode, camera_name

    def _stage_one_camera_scene(self, scene_dir: Path) -> Tuple[Path, Optional[str]]:
        if self.oss_stage_cache is None:
            return scene_dir, None
        scene_uri = self._oss_uri_for_scene(scene_dir)
        if not scene_uri:
            return scene_dir, None
        mode, camera_name = self._choose_long6_fallback_camera()
        staged_camera_dir = self.oss_stage_cache.stage_dir(
            dataset="manip",
            relative_key=f"{scene_dir.name}/{camera_name}",
            oss_uri=join_oss_uri(scene_uri, camera_name),
            dest_root=self._stage_root_for_manip(),
        )
        return staged_camera_dir.parent, mode

    def _sample_staged_camera_entries(
        self,
        entries: Sequence[FrameEntry],
        mode_label: str,
    ) -> Tuple[List[FrameEntry], str]:
        by_view, _by_vt, _wrist_views, _static_views = self._classify_cameras(entries)
        candidate_views = sorted(by_view.keys())
        if mode_label == "W":
            stride_min, stride_max = self.w_stride_min, self.w_stride_max
        elif mode_label == "T":
            stride_min, stride_max = self.moving_stride_min, self.moving_stride_max
        elif mode_label == "F":
            stride_min, stride_max = self.fixed_stride_min, self.fixed_stride_max
        else:
            stride_min, stride_max = self.random_stride_min, self.random_stride_max
        selected = self._sample_single_camera_walk(
            by_view,
            candidate_views,
            stride_min,
            stride_max,
            f"Long6 fallback mode {mode_label}",
        )
    ...
    def _manip_entries_for_scene(self, scene_dir: Path) -> List[FrameEntry]:
        ...
        with os.scandir(scene_dir) as iterator:
            for entry in iterator:
                ...
                if not (camera_dir / "rgb").is_dir() or not (camera_dir / "depth_npy").is_dir():
                    continue
                pose_path = camera_dir / "camera2env_extrinsics.npy"
                intrinsics_path = camera_dir / "intrinsics.npy"
                if pose_path.is_file() and intrinsics_path.is_file():
        ...
        for view_id, (camera_name, camera_dir, pose_path) in enumerate(camera_dirs):
            ...
            image_dir = camera_dir / "rgb"
            depth_dir = camera_dir / "depth_npy"
    ...
    # ------------------------------------------------------------------
    # manip_4d_mixed for Manip_long6: W/T/F single-camera walks.
    # ------------------------------------------------------------------
    ...
    def _sample_single_camera_walk(
        self,
        by_view: Dict[int, List[FrameEntry]],
        candidate_views: Sequence[int],
        stride_min: int,
        stride_max: int,
        label: str,
    ) -> List[FrameEntry]:
        if not candidate_views:
            raise RuntimeError(f"{label}: no candidate cameras")
        view_id = random.choice(list(candidate_views))
        camera_entries = by_view[view_id]
        if not camera_entries:
            raise RuntimeError(f"{label}: empty camera entries")
    ...
    def _is_long6_scene(self, scene_dir: Optional[Path]) -> bool:
        if scene_dir is None or not self.long6_root_marker:
            return False
        return self.long6_root_marker.lower() in str(scene_dir).lower()

    def _long6_surround_views(
        self,
        by_view: Dict[int, List[FrameEntry]],
        static_views: Sequence[int],
    ) -> Tuple[List[int], List[int]]:
        fixed_views: List[int] = []
        moving_views: List[int] = []
        fixed_prefix = self.fixed_camera_prefix.lower()
        moving_prefix = self.moving_camera_prefix.lower()
        for view_id in static_views:
            entries = by_view.get(view_id) or []
            name = (entries[0].camera_name or "").lower() if entries else ""
            if fixed_prefix and name.startswith(fixed_prefix):
                fixed_views.append(view_id)
            elif moving_prefix and name.startswith(moving_prefix):
                moving_views.append(view_id)
        return fixed_views, moving_views

    def _weighted_mode_order(self, modes: Sequence[str], weights: Dict[str, float]) -> List[str]:
        order: List[str] = []
        remaining = list(modes)
        remaining_weights = [weights.get(key, 0.0) for key in remaining]
        while remaining:
    ...
    def _sample_long6_mixed(
        ...
        fixed_views, moving_views = self._long6_surround_views(by_view, static_views)
        all_views = sorted(set(wrist_views) | set(fixed_views) | set(moving_views) | set(static_views))
        enabled_modes = [
            mode for mode in ("W", "T", "F")
            if self.long6_mode_weights.get(mode, 0.0) > 0.0
        ]
        order = self._weighted_mode_order(enabled_modes, self.long6_mode_weights)
        last_error: Optional[Exception] = None
        for mode in order:
            try:
                if mode == "W":
                    return self._sample_single_camera_walk(
                        by_view,
                        list(wrist_views) or all_views,
                        self.w_stride_min,
                        self.w_stride_max,
                        "Long6 mode W",
                    ), "W"
                if mode == "T":
                    return self._sample_single_camera_walk(
                        by_view,
                        moving_views or list(static_views) or all_views,
                        self.moving_stride_min,
                        self.moving_stride_max,
                        "Long6 mode T",
                    ), "T"
                if mode == "F":
                    return self._sample_single_camera_walk(
                        by_view,
                        fixed_views or list(static_views) or all_views,
                        self.fixed_stride_min,
                        self.fixed_stride_max,
                        "Long6 mode F",
                    ), "F"
            except Exception as exc:  # noqa: BLE001 - fall back to next feasible mode
                last_error = exc
                continue
        raise RuntimeError(f"long6 mixed sampling: all modes failed. last_error={last_error}")
    ...
    def _sample_manip_4d_mixed(
        ...
        if not self._is_long6_scene(scene_dir):
            raise ValueError("Manip sampling only supports Manip_long6 scenes")
        by_view, by_vt, wrist_views, static_views = self._classify_cameras(entries)
        return self._sample_long6_mixed(by_view, by_vt, wrist_views, static_views)
    ...
    def _load_camera_for_entry(
        ...
        if cached_pose is None:
            if entry.pose_path.suffix.lower() != ".npy":
                raise ValueError(f"Manip_long6 expects camera2env_extrinsics.npy, got {entry.pose_path}")
            cached_pose = read_manip_long6_camera_arrays(entry.pose_path)
    ...
    def __getitem__(self, index: int) -> Dict[str, object]:
        ...
        for attempt in range(min(16, len(self.scene_dirs))):
            ...
            try:
                ...
                if self.oss_stage_cache is not None and self.oss_stage_cache.should_stage(scene_dir):
                    read_scene_dir, forced_mode = self._stage_one_camera_scene(scene_dir)
                else:
                    read_scene_dir = scene_dir
                entries = self._entries_for_scene(read_scene_dir)
                if forced_mode is not None:
                    selected, mode_label = self._sample_staged_camera_entries(entries, forced_mode)
                else:
                    selected, mode_label = self._sample_entries(entries, read_scene_dir)
                jitter_params = self._sample_color_jitter_params()
                loaded = [self._load_one(read_scene_dir, entry, jitter_params) for entry in selected]
                ...
                if (
                    self.oss_stage_cache is not None
                    and self.oss_stage_cache.enable_fallback_from_exception(exc, source_path=scene_dir)
                ):
                    self._entries_cache.pop(scene_dir, None)
                last_error = exc
                continue
...
def build_dataloaders(args: argparse.Namespace) -> Tuple[DataLoader, Optional[DataLoader], int, int]:
    ...
    oss_stage_cache = build_oss_stage_cache(
        enabled=args.oss_stage_fallback,
        stage_root=args.oss_stage_root,
        mount_root=args.oss_stage_mount_root,
        ossutil_bin=args.ossutil_bin,
        ossutil_config=args.ossutil_config,
        max_entries=args.oss_stage_max_entries,
        ossutil_jobs=args.oss_stage_ossutil_jobs,
        ossutil_checkers=args.oss_stage_ossutil_checkers,
        ossutil_parallel=args.oss_stage_ossutil_parallel,
        delete_workers=args.oss_stage_delete_workers,
        delete_batch=args.oss_stage_delete_batch,
        worker_max_entries=args.oss_stage_worker_max_entries,
        rclone_bin=args.rclone_bin,
        rclone_config=args.rclone_config,
        rclone_remote=args.rclone_remote,
        rclone_transfers=args.oss_stage_rclone_transfers,
        rclone_checkers=args.oss_stage_rclone_checkers,
    )
    oss_uri_roots = parse_str_list(args.oss_uri_roots)
    ...
    common_kwargs = dict(
        ...
        w_stride_min=args.w_stride_min if args.w_stride_min > 0 else None,
        w_stride_max=args.w_stride_max if args.w_stride_max > 0 else None,
        ...
        fixed_stride_min=args.fixed_stride_min,
        fixed_stride_max=args.fixed_stride_max,
        long6_root_marker=args.long6_root_marker,
        long6_mode_weights=parse_mode_weights(args.long6_mode_weights),
        ...
        oss_stage_cache=oss_stage_cache,
        data_roots=args.data_roots,
        oss_uri_roots=oss_uri_roots,
    )
    ...
    dataset_log_rows: List[DatasetLogRow] = [
        DatasetLogRow(
            "train",
            "Manip_long6",
            len(train_dataset),
            _format_views(args.min_sample_frames, args.max_sample_frames),
            args.image_size,
            "x1",
            f"scenes={len(train_scenes):,}",
        )
    ]
    if val_dataset is not None:
        dataset_log_rows.append(
            DatasetLogRow(
                "val",
                "Manip_long6",
                len(val_dataset),
    ...
    if getattr(args, "mixture_curriculum", False):
        for ext_name in ("dl3dv", "scannetpp", "arkitscenes_highres", "hypersim", "tartanair", "dynamic_replica", "adt", "ase", "co3d"):
    ...
    if getattr(args, "dl3dv_root", "") and args.dl3dv_root.strip():
        ...
        dl3dv_train = _init_dataset_quietly(
            DL3DVTrajectoryDataset,
            split="train",
            color_jitter_strength=args.color_jitter_strength,
            color_jitter_prob=args.color_jitter_prob,
            verbose=True,
        ...
        dataset_log_rows.append(DatasetLogRow("train", "DL3DV", len(dl3dv_train), _format_views(dl3dv_min_views, dl3dv_num_views), args.image_size, f"x{args.dl3dv_repeat}", f"video_p={args.dl3dv_video_prob:g}"))
        if getattr(args, "dl3dv_val", False):
            dl3dv_val = _init_dataset_quietly(
            DL3DVTrajectoryDataset,
                split="test",
            ...
            dataset_log_rows.append(DatasetLogRow("val", "DL3DV", len(dl3dv_val), _format_views(dl3dv_min_views, dl3dv_num_views), args.image_size, "x1", ""))
    ...
    if getattr(args, "scannetpp_root", "") and args.scannetpp_root.strip():
        ...
        scannetpp_train = _init_dataset_quietly(
            ScanNetppTrajectoryDataset,
            split="train",
            color_jitter_strength=args.color_jitter_strength,
            color_jitter_prob=args.color_jitter_prob,
            verbose=True,
        ...
        dataset_log_rows.append(DatasetLogRow("train", "ScanNet++", len(scannetpp_train), _format_views(scannetpp_min_views, scannetpp_num_views), args.image_size, f"x{args.scannetpp_repeat}", f"video_p={args.scannetpp_video_prob:g}"))
        if getattr(args, "scannetpp_val", False):
            scannetpp_val = _init_dataset_quietly(
            ScanNetppTrajectoryDataset,
                split="test",
                color_jitter_strength=0.0,
                color_jitter_prob=0.0,
            ...
            dataset_log_rows.append(DatasetLogRow("val", "ScanNet++", len(scannetpp_val), _format_views(scannetpp_min_views, scannetpp_num_views), args.image_size, "x1", ""))

    # ----- ARKitScenesHighRes mix-in (sampling follows base3d-clean/datasets/arkitscenes_highres.py; NOT Manip_long) -----
    if getattr(args, "arkitscenes_highres_root", "") and args.arkitscenes_highres_root.strip():
        from lingbot_map.data.arkitscenes_highres import ARKitScenesHighResTrajectoryDataset
        arkit_num_views = (
            args.arkitscenes_highres_num_views if args.arkitscenes_highres_num_views > 0 else args.max_sample_frames
        )
        arkit_min_views = (
            args.arkitscenes_highres_min_views if args.arkitscenes_highres_min_views > 0 else args.min_sample_frames
        )
        arkit_min_views = min(arkit_min_views, arkit_num_views)
        arkit_common = dict(
            root=args.arkitscenes_highres_root.strip(),
            num_views=arkit_num_views,
            min_views=arkit_min_views,
            image_size=args.image_size,
            patch_size=args.patch_size,
            preprocess_mode=args.preprocess_mode,
            min_depth=args.min_depth,
            max_depth=args.max_depth,
            samples_per_scene=args.samples_per_scene,
            min_interval=args.arkitscenes_highres_min_interval,
            max_interval=args.arkitscenes_highres_max_interval,
            video_prob=args.arkitscenes_highres_video_prob,
            fix_interval_prob=args.arkitscenes_highres_fix_interval_prob,
            block_shuffle=args.arkitscenes_highres_block_shuffle,
        )
        arkit_train = _init_dataset_quietly(
            ARKitScenesHighResTrajectoryDataset,
            split="train",
            color_jitter_strength=args.color_jitter_strength,
            color_jitter_prob=args.color_jitter_prob,
            verbose=True,
        ...
        if args.arkitscenes_highres_repeat > 1:
            train_dataset = torch.utils.data.ConcatDataset(
                [train_dataset] + [arkit_train] * int(args.arkitscenes_highres_repeat)
            )
        else:
            train_dataset = torch.utils.data.ConcatDataset([train_dataset, arkit_train])
        external_train.append(("arkitscenes_highres", len(arkit_train)))
        dataset_log_rows.append(DatasetLogRow("train", "ARKitScenes", len(arkit_train), _format_views(arkit_min_views, arkit_num_views), args.image_size, f"x{args.arkitscenes_highres_repeat}", f"interval={args.arkitscenes_highres_min_interval}-{args.arkitscenes_highres_max_interval}"))
        if getattr(args, "arkitscenes_highres_val", False):
            arkit_val = _init_dataset_quietly(
            ARKitScenesHighResTrajectoryDataset,
                split="test",
                color_jitter_strength=0.0,
                color_jitter_prob=0.0,
            ...
            if val_dataset is None:
                val_dataset = arkit_val
            else:
                val_dataset = torch.utils.data.ConcatDataset([val_dataset, arkit_val])
            dataset_log_rows.append(DatasetLogRow("val", "ARKitScenes", len(arkit_val), _format_views(arkit_min_views, arkit_num_views), args.image_size, "x1", ""))

    # ----- Hypersim mix-in (sampling follows base3d-clean/datasets/hypersim.py; NOT Manip_long) -----
    if getattr(args, "hypersim_root", "") and args.hypersim_root.strip():
        from lingbot_map.data.hypersim import HypersimTrajectoryDataset
        hypersim_num_views = (
            args.hypersim_num_views if args.hypersim_num_views > 0 else args.max_sample_frames
        )
        hypersim_min_views = (
            args.hypersim_min_views if args.hypersim_min_views > 0 else args.min_sample_frames
        )
        hypersim_min_views = min(hypersim_min_views, hypersim_num_views)
        hypersim_common = dict(
            root=args.hypersim_root.strip(),
            num_views=hypersim_num_views,
            min_views=hypersim_min_views,
            image_size=args.image_size,
            patch_size=args.patch_size,
            preprocess_mode=args.preprocess_mode,
            min_depth=args.min_depth,
            max_depth=args.max_depth,
            samples_per_scene=args.samples_per_scene,
            min_interval=args.hypersim_min_interval,
            max_interval=args.hypersim_max_interval,
            video_prob=args.hypersim_video_prob,
            fix_interval_prob=args.hypersim_fix_interval_prob,
            block_shuffle=args.hypersim_block_shuffle,
        )
        hypersim_train = _init_dataset_quietly(
            HypersimTrajectoryDataset,
            split="train",
            color_jitter_strength=args.color_jitter_strength,
            color_jitter_prob=args.color_jitter_prob,
            verbose=True,
        ...
        if args.hypersim_repeat > 1:
            train_dataset = torch.utils.data.ConcatDataset(
                [train_dataset] + [hypersim_train] * int(args.hypersim_repeat)
            )
        else:
            train_dataset = torch.utils.data.ConcatDataset([train_dataset, hypersim_train])
        external_train.append(("hypersim", len(hypersim_train)))
        dataset_log_rows.append(DatasetLogRow("train", "Hypersim", len(hypersim_train), _format_views(hypersim_min_views, hypersim_num_views), args.image_size, f"x{args.hypersim_repeat}", f"interval={args.hypersim_min_interval}-{args.hypersim_max_interval}"))
        if getattr(args, "hypersim_val", False):
            hypersim_val = _init_dataset_quietly(
            HypersimTrajectoryDataset,
                split="test",
                color_jitter_strength=0.0,
                color_jitter_prob=0.0,
            ...
            if val_dataset is None:
                val_dataset = hypersim_val
            else:
                val_dataset = torch.utils.data.ConcatDataset([val_dataset, hypersim_val])
            dataset_log_rows.append(DatasetLogRow("val", "Hypersim", len(hypersim_val), _format_views(hypersim_min_views, hypersim_num_views), args.image_size, "x1", ""))
    ...
    if getattr(args, "tartanair_root", "") and args.tartanair_root.strip():
        ...
        tartanair_train = _init_dataset_quietly(
            TartanAirTrajectoryDataset,
            split="train",
            color_jitter_strength=args.color_jitter_strength,
            color_jitter_prob=args.color_jitter_prob,
            verbose=True,
        ...
        dataset_log_rows.append(DatasetLogRow("train", "TartanAir", len(tartanair_train), _format_views(tartanair_min_views, tartanair_num_views), args.image_size, f"x{args.tartanair_repeat}", f"video_p={args.tartanair_video_prob:g}"))
        if getattr(args, "tartanair_val", False):
            tartanair_val = _init_dataset_quietly(
            TartanAirTrajectoryDataset,
                split="test",
                color_jitter_strength=0.0,
                color_jitter_prob=0.0,
            ...
            dataset_log_rows.append(DatasetLogRow("val", "TartanAir", len(tartanair_val), _format_views(tartanair_min_views, tartanair_num_views), args.image_size, "x1", ""))
    ...
    if getattr(args, "dynamic_replica_root", "") and args.dynamic_replica_root.strip():
        ...
        dynamic_replica_train = _init_dataset_quietly(
            DynamicReplicaTrajectoryDataset,
            split="train",
            color_jitter_strength=args.color_jitter_strength,
            color_jitter_prob=args.color_jitter_prob,
            verbose=True,
        ...
        dataset_log_rows.append(DatasetLogRow("train", "DynamicReplica", len(dynamic_replica_train), _format_views(dynamic_replica_min_views, dynamic_replica_num_views), args.image_size, f"x{args.dynamic_replica_repeat}", f"video_p={args.dynamic_replica_video_prob:g}"))
        if getattr(args, "dynamic_replica_val", False):
            dynamic_replica_val = _init_dataset_quietly(
            DynamicReplicaTrajectoryDataset,
                split="valid",
                color_jitter_strength=0.0,
                color_jitter_prob=0.0,
            ...
            dataset_log_rows.append(DatasetLogRow("val", "DynamicReplica", len(dynamic_replica_val), _format_views(dynamic_replica_min_views, dynamic_replica_num_views), args.image_size, "x1", ""))

    # ----- ADT mix-in (sampling follows base3d-clean/datasets/adt.py; NOT Manip_long) -----
    if getattr(args, "adt_root", "") and args.adt_root.strip():
        from lingbot_map.data.adt import ADTTrajectoryDataset
        adt_num_views = args.adt_num_views if args.adt_num_views > 0 else args.max_sample_frames
        adt_min_views = args.adt_min_views if args.adt_min_views > 0 else args.min_sample_frames
        adt_min_views = min(adt_min_views, adt_num_views)
        adt_common = dict(
            root=args.adt_root.strip(),
            num_views=adt_num_views,
            min_views=adt_min_views,
            image_size=args.image_size,
            patch_size=args.patch_size,
            preprocess_mode=args.preprocess_mode,
            min_depth=args.min_depth,
            max_depth=args.max_depth,
            samples_per_scene=args.samples_per_scene,
            max_distance=args.adt_max_distance,
            rotate_clockwise=args.adt_rotate_clockwise,
        )
        adt_train = _init_dataset_quietly(
            ADTTrajectoryDataset,
            split="train",
            color_jitter_strength=args.color_jitter_strength,
            color_jitter_prob=args.color_jitter_prob,
            verbose=True,
        ...
        if args.adt_repeat > 1:
            train_dataset = torch.utils.data.ConcatDataset(
                [train_dataset] + [adt_train] * int(args.adt_repeat)
            )
        else:
            train_dataset = torch.utils.data.ConcatDataset([train_dataset, adt_train])
        external_train.append(("adt", len(adt_train)))
        dataset_log_rows.append(DatasetLogRow("train", "ADT", len(adt_train), _format_views(adt_min_views, adt_num_views), args.image_size, f"x{args.adt_repeat}", f"dist={args.adt_max_distance},rot={int(args.adt_rotate_clockwise)}"))
        if getattr(args, "adt_val", False):
            adt_val = _init_dataset_quietly(
            ADTTrajectoryDataset,
                split="test",  # ADT has no separate split here; reuse the same scene pool.
                color_jitter_strength=0.0,
                color_jitter_prob=0.0,
                verbose=True,
                **adt_common,
            ...
            if val_dataset is None:
                ...
                val_dataset = torch.utils.data.ConcatDataset([val_dataset, adt_val])
            dataset_log_rows.append(DatasetLogRow("val", "ADT", len(adt_val), _format_views(adt_min_views, adt_num_views), args.image_size, "x1", ""))

    # ----- ASE mix-in (sampling follows base3d-clean/datasets/ase_20251008.py; NOT Manip_long) -----
    if getattr(args, "ase_root", "") and args.ase_root.strip():
        from lingbot_map.data.ase import ASETrajectoryDataset
        ase_num_views = args.ase_num_views if args.ase_num_views > 0 else args.max_sample_frames
        ase_min_views = args.ase_min_views if args.ase_min_views > 0 else args.min_sample_frames
        ase_min_views = min(ase_min_views, ase_num_views)
        ase_common = dict(
            root=args.ase_root.strip(),
            num_views=ase_num_views,
            min_views=ase_min_views,
            image_size=args.image_size,
            patch_size=args.patch_size,
            preprocess_mode=args.preprocess_mode,
            min_depth=args.min_depth,
            max_depth=args.max_depth,
            samples_per_scene=args.samples_per_scene,
            max_distance=args.ase_max_distance,
            cache_path=args.ase_cache_path.strip() or None,
        )
        ase_train = _init_dataset_quietly(
            ASETrajectoryDataset,
            split="train",
            color_jitter_strength=args.color_jitter_strength,
            color_jitter_prob=args.color_jitter_prob,
            verbose=True,
        ...
        if args.ase_repeat > 1:
            train_dataset = torch.utils.data.ConcatDataset(
                [train_dataset] + [ase_train] * int(args.ase_repeat)
            )
        else:
            train_dataset = torch.utils.data.ConcatDataset([train_dataset, ase_train])
        external_train.append(("ase", len(ase_train)))
        dataset_log_rows.append(DatasetLogRow("train", "ASE", len(ase_train), _format_views(ase_min_views, ase_num_views), args.image_size, f"x{args.ase_repeat}", f"dist={args.ase_max_distance}"))
        if getattr(args, "ase_val", False):
            ase_val = _init_dataset_quietly(
            ASETrajectoryDataset,
                split="test",  # ASE has no separate split here; reuse the same scene pool.
                color_jitter_strength=0.0,
                color_jitter_prob=0.0,
                verbose=True,
                **ase_common,
            ...
            if val_dataset is None:
                ...
                val_dataset = torch.utils.data.ConcatDataset([val_dataset, ase_val])
            dataset_log_rows.append(DatasetLogRow("val", "ASE", len(ase_val), _format_views(ase_min_views, ase_num_views), args.image_size, "x1", ""))

    # ----- CO3D mix-in (sampling follows base3d-clean/datasets/co3d_20251006.py; NOT Manip_long) -----
    if getattr(args, "co3d_root", "") and args.co3d_root.strip():
        from lingbot_map.data.co3d import Co3dTrajectoryDataset
        co3d_num_views = args.co3d_num_views if args.co3d_num_views > 0 else args.max_sample_frames
        co3d_min_views = args.co3d_min_views if args.co3d_min_views > 0 else args.min_sample_frames
        co3d_min_views = min(co3d_min_views, co3d_num_views)
        co3d_common = dict(
            root=args.co3d_root.strip(),
            num_views=co3d_num_views,
            min_views=co3d_min_views,
            image_size=args.image_size,
            patch_size=args.patch_size,
            preprocess_mode=args.preprocess_mode,
            min_depth=args.min_depth,
            max_depth=args.max_depth,
            samples_per_scene=args.samples_per_scene,
            mask_bg=args.co3d_mask_bg,
            base3d_root=args.co3d_base3d_root,
            oss_stage_cache=oss_stage_cache,
            oss_uri_root=args.co3d_oss_uri_root,
        )
        co3d_train = _init_dataset_quietly(
            Co3dTrajectoryDataset,
            split="train",
            color_jitter_strength=args.color_jitter_strength,
            color_jitter_prob=args.color_jitter_prob,
            verbose=True,
        ...
        if args.co3d_repeat > 1:
            train_dataset = torch.utils.data.ConcatDataset(
                [train_dataset] + [co3d_train] * int(args.co3d_repeat)
            )
        else:
            train_dataset = torch.utils.data.ConcatDataset([train_dataset, co3d_train])
        external_train.append(("co3d", len(co3d_train)))
        dataset_log_rows.append(DatasetLogRow("train", "CO3D", len(co3d_train), _format_views(co3d_min_views, co3d_num_views), args.image_size, f"x{args.co3d_repeat}", f"mask_bg={args.co3d_mask_bg}"))
        if getattr(args, "co3d_val", False):
            co3d_val = _init_dataset_quietly(
            Co3dTrajectoryDataset,
                split="test",
                color_jitter_strength=0.0,
                color_jitter_prob=0.0,
            ...
            if val_dataset is None:
                val_dataset = co3d_val
            else:
                val_dataset = torch.utils.data.ConcatDataset([val_dataset, co3d_val])
            dataset_log_rows.append(DatasetLogRow("val", "CO3D", len(co3d_val), _format_views(co3d_min_views, co3d_num_views), args.image_size, "x1", ""))

    _print_dataset_mix_table(dataset_log_rows)
    ...
    if getattr(args, "mixture_curriculum", False):
        if not external_train:
            ...
            ext_weight_map = parse_weight_map(args.mixture_external_weights)
            ext_weights = [ext_weight_map.get(name, 1.0) for name in ext_names] if ext_weight_map else None
            ...
            if ext_weight_map:
                print(f"[mixture] external relative weights: {dict(zip(ext_names, ext_weights or []))}")
            print(
    ...
    if args.num_workers > 0:
        dataloader_worker_kwargs["prefetch_factor"] = max(1, int(args.dataloader_prefetch_factor))
    ...
    val_loader = None
    if val_dataset is not None:
        val_num_workers = max(1, min(args.num_workers, 4)) if args.num_workers > 0 else 0
        val_worker_kwargs = {}
        if val_num_workers > 0:
            val_worker_kwargs["prefetch_factor"] = max(1, int(args.dataloader_prefetch_factor))
...
def build_model(args: argparse.Namespace, device: torch.device) -> nn.Module:
    ...
    model = GCTStream(
        ...
        camera_activation_checkpoint=args.camera_activation_checkpoint,
    )
    ...
    use_camera_ckpt = args.camera_activation_checkpoint
    if getattr(model, "camera_head", None) is not None and hasattr(model.camera_head, "use_activation_checkpoint"):
        model.camera_head.use_activation_checkpoint = use_camera_ckpt
        print(f"[model] camera activation checkpoint: {int(use_camera_ckpt)}")
...
def catastrophic_loss_reason(losses: Dict[str, torch.Tensor], args: argparse.Namespace) -> Optional[str]:
    thresholds = {
        "loss_objective": getattr(args, "skip_loss_objective_threshold", 0.0),
        "loss_relative_trans": getattr(args, "skip_loss_relative_trans_threshold", 0.0),
        "loss_T": getattr(args, "skip_loss_t_threshold", 0.0),
        "loss_R": getattr(args, "skip_loss_r_threshold", 0.0),
    }
    triggered: List[str] = []
    for key, threshold in thresholds.items():
        ...
        if math.isfinite(scalar) and scalar > float(threshold):
            triggered.append(f"{key}={scalar:.6g}>{float(threshold):.6g}")
...
def train(args: argparse.Namespace) -> None:
    ...
    profile_end_step = initial_global_step + max(0, args.profile_warmup_steps) + max(0, args.profile_steps)
    profile_first_step = initial_global_step + max(0, args.profile_warmup_steps) + 1
    ...
    def profile_add(name: str, value: float) -> None:
        profile_totals[name] = profile_totals.get(name, 0.0) + float(value)

    profile_prev_end = profile_now()
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            profile_step_start = profile_now()
            profile_data_wait = profile_step_start - profile_prev_end
            profile_lap = profile_step_start
            if args.canonicalize_first_frame:
                batch = canonicalize_to_first_frame(batch)
            if args.normalize_scene:
                batch = normalize_scene_batch(
            ...
            profile_after_preprocess = profile_now()
            batch = to_device(batch, device)
            profile_after_to_device = profile_now()
            ...
            with torch.amp.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
                ...
                profile_after_model = profile_now()
                losses = criterion(predictions, batch)
                loss = losses["objective"] / max(1, args.accum_steps)
            profile_after_forward_loss = profile_now()
            ...
            catastrophic_reason = catastrophic_loss_reason(losses, args)
            if catastrophic_reason is not None:
                src = format_batch_source(batch)
                mode = format_batch_mode(batch)
                tqdm.write(
                    f"[warn] catastrophic finite loss at step={global_step}, skipping batch: "
                    f"{catastrophic_reason}; source={src}; mode={mode}"
                )
                model.clean_kv_cache()
                optimizer.zero_grad(set_to_none=True)
                profile_prev_end = profile_now()
            ...
            if scaler.is_enabled():
                ...
                loss.backward()
            model.clean_kv_cache()
            profile_after_backward = profile_now()
            ...
            if should_step:
                ...
                scheduler.step()
                global_step += 1
                profile_after_optimizer = profile_now()
                if (
                    device.type == "cuda"
                ...
                profile_after_empty_cache = profile_now()
                ...
                if (
                    ...
                    tb_writer.flush()
                profile_after_metrics = profile_now()

                if profile_enabled and profile_first_step <= global_step <= profile_end_step:
                    timings = {
                        "data_wait": profile_data_wait,
                        "preprocess": profile_after_preprocess - profile_lap,
                        "to_device": profile_after_to_device - profile_after_preprocess,
                        "model_forward": profile_after_model - profile_after_to_device,
                        "loss_compute": profile_after_forward_loss - profile_after_model,
                        "forward_loss": profile_after_forward_loss - profile_after_to_device,
                        "backward": profile_after_backward - profile_after_forward_loss,
                        "optimizer": profile_after_optimizer - profile_after_backward,
                        "empty_cache": profile_after_empty_cache - profile_after_optimizer,
                        "metrics_tb": profile_after_metrics - profile_after_empty_cache,
                        "total": profile_after_metrics - profile_prev_end,
                    }
                    for key, value in timings.items():
                        profile_add(key, value)
                    profile_count += 1
                    if args.profile_print_every <= 1 or profile_count % args.profile_print_every == 0:
                        src = format_batch_source(batch)
                        mode = format_batch_mode(batch)
                        timing_text = " ".join(f"{key}={value:.3f}s" for key, value in timings.items())
                        tqdm.write(
                            f"[profile] step={global_step} {timing_text} "
                            f"src={src} mode={mode or '-'} input={input_desc}"
                        )
                    if global_step == profile_end_step and profile_count > 0:
                        avg = {key: value / profile_count for key, value in profile_totals.items()}
                        total_avg = max(avg.get("total", 0.0), 1e-9)
                        avg_text = " ".join(
                            f"{key}={value:.3f}s/{(value / total_avg * 100.0):.1f}%"
                            for key, value in avg.items()
                        )
                        tqdm.write(f"[profile-summary] steps={profile_count} avg {avg_text}")
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--oss_stage_fallback", action=argparse.BooleanOptionalAction, default=False, help="Stage OSS-backed trajectories to CPFS after /oss mount failures.")
    parser.add_argument("--oss_stage_root", type=str, default="/cpfs/user/guowenqi/lingbot-map-oss-stage")
    parser.add_argument("--oss_stage_mount_root", type=str, default="/oss-guowenqi")
    parser.add_argument("--oss_stage_max_entries", type=int, default=1000, help="Maximum staged Manip_long6 + CO3D trajectories kept on CPFS.")
    parser.add_argument("--oss_stage_ossutil_jobs", type=int, default=64, help="ossutil cp -j value for fallback staging.")
    parser.add_argument("--oss_stage_ossutil_checkers", type=int, default=128, help="ossutil cp --checkers value for fallback staging.")
    parser.add_argument("--oss_stage_ossutil_parallel", type=int, default=4, help="ossutil cp --parallel value for fallback staging.")
    parser.add_argument("--oss_stage_delete_workers", type=int, default=8, help="Parallel rm workers for stage cache eviction.")
    parser.add_argument("--oss_stage_delete_batch", type=int, default=512, help="Files per rm invocation during parallel stage cache eviction.")
    parser.add_argument("--oss_stage_worker_max_entries", type=int, default=25, help="Maximum staged entries kept by each DataLoader worker cache.")
    parser.add_argument("--rclone_bin", type=str, default=DEFAULT_RCLONE_BIN, help="Deprecated compatibility argument; ossutil is used for fallback staging.")
    parser.add_argument("--rclone_config", type=str, default=DEFAULT_RCLONE_CONFIG, help="Deprecated compatibility argument; ossutil is used for fallback staging.")
    parser.add_argument("--rclone_remote", type=str, default="aliyunoss", help="Deprecated compatibility argument; ossutil is used for fallback staging.")
    parser.add_argument("--oss_stage_rclone_transfers", type=int, default=None, help="Deprecated compatibility argument; ossutil is used for fallback staging.")
    parser.add_argument("--oss_stage_rclone_checkers", type=int, default=None, help="Deprecated compatibility argument; ossutil is used for fallback staging.")
    ...
    # ARKitScenesHighRes mix-in (sampling follows base3d-clean/datasets/arkitscenes_highres.py; NOT Manip_long).
    parser.add_argument(
        "--arkitscenes_highres_root",
        type=str,
        default="",
        help="ARKitScenesHighRes root containing Training/Validation scene dirs. Empty disables ARKitScenesHighRes.",
    )
    parser.add_argument(
        "--arkitscenes_highres_num_views",
        type=int,
        default=0,
        help="Maximum per-sample ARKitScenesHighRes clip length. 0 follows --max_sample_frames.",
    )
    parser.add_argument(
        "--arkitscenes_highres_min_views",
        type=int,
        default=0,
        help="Minimum ARKitScenesHighRes clip length. 0 follows --min_sample_frames.",
    )
    parser.add_argument(
        "--arkitscenes_highres_repeat",
        type=int,
        default=1,
        help="Repeat ARKitScenesHighRes N times in ConcatDataset when mixture curriculum is disabled.",
    )
    parser.add_argument(
        "--arkitscenes_highres_val",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Also mix ARKitScenesHighRes Validation split into validation.",
    )
    # Defaults match base3d-clean/datasets/arkitscenes_highres.py.
    parser.add_argument("--arkitscenes_highres_min_interval", type=int, default=1)
    parser.add_argument("--arkitscenes_highres_max_interval", type=int, default=32)
    parser.add_argument("--arkitscenes_highres_video_prob", type=float, default=0.5)
    parser.add_argument("--arkitscenes_highres_fix_interval_prob", type=float, default=0.5)
    parser.add_argument("--arkitscenes_highres_block_shuffle", type=int, default=16)
    # Hypersim mix-in (sampling follows base3d-clean/datasets/hypersim.py; NOT Manip_long).
    parser.add_argument(
        "--hypersim_root",
        type=str,
        default="",
        help="Hypersim root containing ai*/{rgb,depth,pose}. Empty disables Hypersim.",
    )
    parser.add_argument(
        "--hypersim_num_views",
        type=int,
        default=0,
        help="Maximum per-sample Hypersim clip length. 0 follows --max_sample_frames.",
    )
    parser.add_argument(
        "--hypersim_min_views",
        type=int,
        default=0,
        help="Minimum Hypersim clip length. 0 follows --min_sample_frames.",
    )
    parser.add_argument(
        "--hypersim_repeat",
        type=int,
        default=1,
        help="Repeat Hypersim N times in ConcatDataset when mixture curriculum is disabled.",
    )
    parser.add_argument(
        "--hypersim_val",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Also mix Hypersim into validation using the same scene pool.",
    )
    # Defaults match base3d-clean/datasets/hypersim.py: only max_interval=4
    # and block_shuffle=16 are specified there; the probabilities are inherited
    # from BaseMultiViewDataset defaults.
    parser.add_argument("--hypersim_min_interval", type=int, default=1)
    parser.add_argument("--hypersim_max_interval", type=int, default=4)
    parser.add_argument("--hypersim_video_prob", type=float, default=0.5)
    parser.add_argument("--hypersim_fix_interval_prob", type=float, default=0.5)
    parser.add_argument("--hypersim_block_shuffle", type=int, default=16)
    # TartanAir mix-in (sampling follows base3d-clean/datasets/tartanair.py).
    parser.add_argument(
        "--tartanair_root",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Root dir containing ADT scene subdirs plus adt_cache.npy (e.g. /shared/smartbot/renkerui/data/adt). Empty disables ADT.",
    )
    parser.add_argument(
        "--adt_num_views",
        type=int,
        default=0,
        help="Per-sample view count for ADT. 0 means follow --max_sample_frames.",
    )
    parser.add_argument(
        "--adt_min_views",
        type=int,
        default=0,
        help="Minimum per-sample ADT view count. 0 means follow --min_sample_frames.",
    )
    parser.add_argument(
        "--adt_repeat",
        type=int,
        default=1,
        help="Repeat the ADT dataset N times in the ConcatDataset to bias the mix ratio.",
    )
    parser.add_argument(
        "--adt_val",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Also mix ADT into the val DataLoader (uses the same scene pool).",
    )
    parser.add_argument(
        "--adt_max_distance",
        type=int,
        default=128,
        help="ADT local-window sampling distance from base3d-clean/datasets/adt.py.",
    )
    parser.add_argument(
        "--adt_rotate_clockwise",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Rotate ADT RGB-D frames clockwise and update intrinsics/camera pose.",
    )
    # ASE mix-in (sampling follows base3d-clean/datasets/ase_20251008.py; NOT Manip_long).
    parser.add_argument(
        "--ase_root",
        type=str,
        default="",
        help="Root dir containing ASE processed scene subdirs plus ase_cache.npy (e.g. /oss-guowenqi/ase_processed). Empty disables ASE.",
    )
    parser.add_argument(
        "--ase_cache_path",
        type=str,
        default="",
        help="Optional ASE cache .npy path. When set, overrides <ase_root>/ase_cache.npy.",
    )
    parser.add_argument(
        "--ase_num_views",
        type=int,
        default=0,
        help="Per-sample view count for ASE. 0 means follow --max_sample_frames.",
    )
    parser.add_argument(
        "--ase_min_views",
        type=int,
        default=0,
        help="Minimum per-sample ASE view count. 0 means follow --min_sample_frames.",
    )
    parser.add_argument(
        "--ase_repeat",
        type=int,
        default=1,
        help="Repeat the ASE dataset N times in the ConcatDataset to bias the mix ratio.",
    )
    parser.add_argument(
        "--ase_val",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Also mix ASE into the val DataLoader (uses the same scene pool).",
    )
    parser.add_argument(
        "--ase_max_distance",
        type=int,
        default=32,
        help="ASE local-window sampling distance from base3d-clean/datasets/ase_20251008.py.",
    )
    # CO3D mix-in (sampling follows base3d-clean/datasets/co3d_20251006.py; NOT Manip_long).
    parser.add_argument(
        "--co3d_root",
        type=str,
        default="",
        help="Root dir for CO3Dv2. Empty disables CO3D.",
    )
    parser.add_argument(
        "--co3d_num_views",
        type=int,
        default=0,
        help="Per-sample max view count for CO3D. 0 means follow --max_sample_frames.",
    )
    parser.add_argument(
        "--co3d_min_views",
        type=int,
        default=0,
        help="Per-sample min view count for CO3D. 0 means follow --min_sample_frames.",
    )
    parser.add_argument(
        "--co3d_repeat",
        type=int,
        default=1,
        help="Repeat the CO3D dataset N times in the ConcatDataset to bias the mix ratio.",
    )
    parser.add_argument(
        "--co3d_val",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Also mix CO3D test split into the val DataLoader.",
    )
    parser.add_argument(
        "--co3d_mask_bg",
        type=str,
        choices=["true", "false", "rand"],
        default="rand",
        help="CO3D mask_bg passed to co3d_20251006.py.",
    )
    parser.add_argument(
        "--co3d_base3d_root",
        type=str,
        default="/cpfs/user/guowenqi/base3d-clean",
        help="Repo root used to import datasets/co3d_20251006.py for CO3D sampling.",
    )
    parser.add_argument(
        "--co3d_oss_uri_root",
        type=str,
        default="oss://pjlab-bjpai-sim/guowenqi/CO3Dv2",
        help="OSS URI root for CO3Dv2 sequence staging.",
    )
    ...
    # p_manip(step), else from one enabled external by relative weights.
    parser.add_argument(
        "--mixture_curriculum",
        action=argparse.BooleanOptionalAction,
        default=False,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.60,
        help="Manip draw probability at step <= --mixture_warmup_start. 1 - p is split by --mixture_external_weights.",
    )
    parser.add_argument(
        "--mixture_p_manip_end",
        type=float,
    ...
    parser.add_argument(
        "--mixture_external_weights",
        type=str,
        default=(
            "dl3dv=4,scannetpp=4,arkitscenes_highres=3.5,"
            "hypersim=2.5,adt=2.5,ase=2.5,dynamic_replica=2,tartanair=1.5,co3d=1.5"
        ),
        help="Comma-separated relative weights for enabled external datasets. Missing enabled externals default to 1.",
    )
    ...
    parser.add_argument("--w_stride_min", type=int, default=0,
                        help="Mode W wrist min frame stride. <=0 inherits --random_stride_min.")
    parser.add_argument("--w_stride_max", type=int, default=0,
                        help="Mode W wrist max frame stride. <=0 inherits --random_stride_max.")
    ...
    # manip_4d_mixed (W/T/F) sampling configuration
    ...
    parser.add_argument("--moving_stride_min", type=int, default=2,
                        help="Manip_long6 moving surround camera min frame stride (~0.07s @ 30fps).")
    parser.add_argument("--moving_stride_max", type=int, default=6,
                        help="Manip_long6 moving surround camera max frame stride (~0.20s @ 30fps).")
    parser.add_argument("--fixed_stride_min", type=int, default=4,
                        help="Manip_long6 fixed surround camera min frame stride (~0.13s @ 30fps).")
    parser.add_argument("--fixed_stride_max", type=int, default=16,
                        help="Manip_long6 fixed surround camera max frame stride (~0.53s @ 30fps).")
    parser.add_argument("--long6_root_marker", type=str, default="Manip_long6",
                        help="Substring used to detect Manip_long6 scenes by path and enable "
                             "Long6 W/T/F camera-specific sampling.")
    parser.add_argument("--long6_mode_weights", type=str, default="W=0.40,T=0.45,F=0.15",
                        help="Manip_long6 mode weights. W=wrist, T=moving surround, "
                             "F=fixed surround.")
    parser.add_argument("--moving_camera_prefix", type=str, default="surround_cam_moving",
                        help="Camera-name prefix for Manip_long6 moving surround camera.")
    parser.add_argument("--fixed_camera_prefix", type=str, default="surround_cam_fixed",
                        help="Camera-name prefix for Manip_long6 fixed surround camera.")
    ...
    parser.add_argument("--depth_frames_chunk_size", type=int, default=16, help="Frames per DPT depth-head chunk; smaller saves memory but is slower")
    parser.add_argument("--depth_activation_checkpoint", dest="no_depth_activation_checkpoint", action="store_false", default=True, help="Enable activation checkpointing in the DPT depth head")
    parser.add_argument("--no_depth_activation_checkpoint", dest="no_depth_activation_checkpoint", action="store_true", help="Disable activation checkpointing in the DPT depth head")
    parser.add_argument("--camera_activation_checkpoint", dest="camera_activation_checkpoint", action="store_true", default=False, help="Enable activation checkpointing in the camera head")
    parser.add_argument("--no_camera_activation_checkpoint", dest="camera_activation_checkpoint", action="store_false", help="Disable activation checkpointing in the camera head")
    ...
    parser.add_argument("--skip_loss_objective_threshold", type=float, default=50.0, help="Skip a finite training batch before backward when loss_objective exceeds this value; 0 disables")
    parser.add_argument("--skip_loss_relative_trans_threshold", type=float, default=10.0, help="Skip a finite training batch before backward when loss_relative_trans exceeds this value; 0 disables")
    parser.add_argument("--skip_loss_t_threshold", type=float, default=10.0, help="Skip a finite training batch before backward when loss_T exceeds this value; 0 disables")
    parser.add_argument("--skip_loss_r_threshold", type=float, default=1.0, help="Skip a finite training batch before backward when loss_R exceeds this value; 0 disables")
    ...
    parser.add_argument("--profile_steps", type=int, default=0, help="Print synchronized per-step timing for this many optimizer steps; 0 disables profiling")
    parser.add_argument("--profile_warmup_steps", type=int, default=0, help="Skip this many optimizer steps before collecting profile timings")
    parser.add_argument("--profile_print_every", type=int, default=1, help="Print every N collected profile steps")
