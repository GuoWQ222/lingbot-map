...
OUTPUT_DIR="${WINDOW32_OUTPUT_DIR:-${REPO_DIR}/outputs/runs/manip_long_train_window32_16gpu}"
...
[mixture curriculum]  (cross-dataset Manip-vs-external sampler)
  ...
  externals     : DL3DV, ScanNet++, TartanAir, DynamicReplica (each enabled iff its *_ROOT is set)
...
exec "${PYTHON_CMD[@]}" -m torch.distributed.run \
  ...
  "${REPO_DIR}/scripts/train/train_multinode.py" \
  "${ARGS[@]}" \
  "$@"
