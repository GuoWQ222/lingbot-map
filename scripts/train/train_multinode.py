...
def train_distributed(args: argparse.Namespace) -> None:
    ...
    step_progress = tqdm(
        ...
        desc=single._cyan(single._bold("train")),
        unit="step",
        dynamic_ncols=True,
        bar_format=(
            "{desc} {percentage:3.0f}%|{bar}| "
            f"{single._dim('{n_fmt}/{total_fmt}')} "
            f"[{single._dim('{elapsed}<{remaining}, {rate_fmt}')}] "
            "{postfix}"
        ),
        colour="cyan",
        smoothing=0.1,
    ...
    while True:
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with sync_context:
                ...
                catastrophic_reason = single.catastrophic_loss_reason(losses, args)
                loss_is_safe = all_ranks_boolean(catastrophic_reason is None, device)
                if not loss_is_safe:
                    if catastrophic_reason is not None:
                        src = single.format_batch_source(batch)
                        mode = single.format_batch_mode(batch)
                        tqdm.write(
                            f"[rank {global_rank}] catastrophic finite loss at step={global_step}, "
                            f"skipping synchronized batch: {catastrophic_reason}; source={src}; mode={mode}"
                        )
                    if global_rank == 0:
                        tqdm.write("[warn] catastrophic finite loss on at least one rank, skipping synchronized batch")
                    model_for_state.clean_kv_cache()
                    optimizer.zero_grad(set_to_none=True)
                    continue
            ...
            if should_step:
                ...
                if global_rank == 0:
                    ...
                    if sampler_obj is not None:
                        if hasattr(sampler_obj, "get_p_manip"):
                            manip_ratio = float(sampler_obj.get_p_manip(global_step))
                        if hasattr(sampler_obj, "get_dataset_weights"):
                    ...
                    postfix = (
                        f"{single._dim('loss')} {single._yellow(f'{current_loss:.4f}')}"
                        f"  {single._dim('lr')} {single._green(f'{lr:.2e}')}"
                        f"  {single._dim('seq')} {seq_len}"
                        f"  {single._dim('ep')} {epoch + 1}"
                    )
                    if manip_ratio is not None:
                        postfix += f"  {single._dim('pManipLong')} {single._magenta(f'{manip_ratio:.3f}')}"
                    ...
                    if args.log_every > 0 and global_step % args.log_every == 0 and running_count > 0:
                        ...
                        if last_logged_loss is None:
                            delta_str = single._dim("Δ----")
                        else:
                            delta = avg_loss - last_logged_loss
                            sign = "↓" if delta < 0 else ("↑" if delta > 0 else "·")
                            color = single._green if delta < 0 else (single._red if delta > 0 else single._dim)
                            delta_str = color(f"{sign}{abs(delta):.4f}")
                        last_logged_loss = avg_loss
                        src = single.format_batch_source(batch)
                        mode = single.format_batch_mode(batch)
                        seq_len = int(batch["images"].shape[1]) if torch.is_tensor(batch.get("images")) else 0
                        seq_field = f"{single._dim('seq')} {seq_len}"
                        if mode:
                            seq_field += f" {single._magenta(mode)}"
                        ts = time.strftime("%H:%M:%S")
                        sep = single._SEP()
                        parts = [
                            f"{single._dim('[' + ts + ']')}",
                            f"{single._bold('step')} {single._cyan(f'{global_step}')}{single._dim('/' + str(total_train_steps))}",
                            f"{single._dim('ep')} {epoch + 1}",
                            f"{single._dim('loss')} {single._yellow(f'{avg_loss:.4f}')} {delta_str}",
                        ]
                        components = single.format_loss_components(averaged)
                        if components:
                            parts.append(components)
                        parts.extend([
                            f"{single._dim('lr')} {single._green(f'{lr:.2e}')}",
                            seq_field,
                            f"{single._dim('pManipLong')} {single._magenta(f'{manip_ratio:.3f}')}" if manip_ratio is not None else "",
                            f"{single._dim('src')} {single._blue(src)}",
                            f"{single._dim('t')} {single._fmt_duration(elapsed)}{single._dim('/eta')} {single._fmt_duration(eta)}",
                        ])
