#!/usr/bin/env python3
"""Visualize one DL3DV trajectory sampled by the training DataLoader."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sample one DL3DV training trajectory and visualize its RGB-D point cloud."
    )
    parser.add_argument(
        "--root",
        default="/cpfs/shared/landmark/renkerui/data/dl3dv",
        help="DL3DV root. Defaults to DL3DV_ROOT in train.sh.",
    )
...
def make_dataloader(args: argparse.Namespace) -> Tuple[DataLoader, DL3DVTrajectoryDataset, int]:
    # These defaults mirror train.sh and build_dataloaders() in train.py.
    dataset = DL3DVTrajectoryDataset(
        root=args.root,
        split="train",
    ...
    if args.scene:
        if args.scene not in dataset.sequences:
            raise ValueError(
                f"Scene {args.scene!r} is not in the DL3DV training dataset "
...
def run_viser(args: argparse.Namespace, batch: Dict[str, object]) -> None:
    ...
    with server.gui.add_folder("DL3DV trajectory"):
