#!/usr/bin/env python3
"""Visualize one ASE trajectory sampled by the training DataLoader."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sample one ASE training trajectory and visualize its RGB-D point cloud."
    )
    parser.add_argument(
        "--root",
        default="/oss-guowenqi/ase_processed",
        help="Processed ASE root.",
    )
    ...
    parser.add_argument("--max-distance", type=int, default=32)
    parser.add_argument(
        "--training-transform",
        action="store_true",
...
def make_dataloader(args: argparse.Namespace) -> Tuple[DataLoader, ASETrajectoryDataset, int]:
    # These defaults mirror train.sh and build_dataloaders() in train.py.
    dataset = ASETrajectoryDataset(
        root=args.root,
        split="train",
    ...
    if args.scene:
        if args.scene not in dataset.sequences:
            raise ValueError(
                f"Scene {args.scene!r} is not in the ASE training dataset "
...
def run_viser(args: argparse.Namespace, batch: Dict[str, object]) -> None:
    ...
    with server.gui.add_folder("ASE trajectory"):
