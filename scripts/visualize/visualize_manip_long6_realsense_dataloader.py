#!/usr/bin/env python3
"""Visualize one Manip_long6 realsense trajectory sampled by the training DataLoader."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sample one Manip_long6 realsense training trajectory and visualize "
    ...
    parser.add_argument(
        ...
        default=False,
        help="Use exported masks if present. Default is false because Manip_long6 realsense exports do not include masks.",
    )

    parser.add_argument(
        "--camera-names",
        default="realsense_left,realsense_right",
        help="Comma-separated Manip_long6 camera dirs to allow.",
    )
    parser.add_argument("--w-stride-min", type=int, default=2)
    parser.add_argument("--w-stride-max", type=int, default=8)
...
def make_dataloader(args: argparse.Namespace) -> Tuple[DataLoader, ManipTrajectoryDataset, int, List[Path]]:
    ...
    dataset = ManipTrajectoryDataset(
        ...
        random_stride_min=2,
        random_stride_max=8,
        w_stride_min=args.w_stride_min,
        w_stride_max=args.w_stride_max,
        ...
        long6_mode_weights={"W": 1.0, "T": 0.0, "F": 0.0, "M": 0.0},
...
def run_viser(args: argparse.Namespace, batch: Dict[str, object]) -> None:
    ...
    with server.gui.add_folder("Manip_long6 realsense trajectory"):
