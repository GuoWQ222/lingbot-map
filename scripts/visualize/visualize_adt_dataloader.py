#!/usr/bin/env python3
"""Visualize one ADT trajectory sampled by the training DataLoader."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sample one ADT training trajectory and visualize its RGB-D point cloud."
    )
    parser.add_argument(
        "--root",
        default="/shared/smartbot/renkerui/data/adt",
        help="ADT root. Defaults to ADT_ROOT in train.sh.",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Rotate ADT RGB-D frames clockwise and update camera geometry.",
    )
    parser.add_argument(
        "--training-transform",
        action="store_true",
...
def make_dataloader(args: argparse.Namespace) -> Tuple[DataLoader, ADTTrajectoryDataset, int]:
    # These defaults mirror train.sh and build_dataloaders() in train.py.
    dataset = ADTTrajectoryDataset(
        root=args.root,
        split="train",
    ...
    if args.scene:
        if args.scene not in dataset.sequences:
            raise ValueError(
                f"Scene {args.scene!r} is not in the ADT training dataset "
...
def run_viser(args: argparse.Namespace, batch: Dict[str, object]) -> None:
    ...
    with server.gui.add_folder("ADT trajectory"):
