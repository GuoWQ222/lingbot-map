#!/usr/bin/env python3
"""Visualize one eval_scal3r-native GT RGB-D batch with Viser."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the same Scal3R-native eval batch used by eval_scal3r.py and "
            "visualize the synchronized GT point cloud."
        )
    )
    parser.add_argument(
        "--train-args-json",
    ...
    parser.add_argument("--output-dir", default="/tmp/eval_scal3r_gt_batch_vis")
    parser.add_argument("--scal3r-repo", default="/cpfs/user/guowenqi/Scal3R")
    parser.add_argument("--split", choices=["val", "train", "all"], default="val")
    parser.add_argument("--max-scenes-eval", type=int, default=1)
    parser.add_argument(
        "--eval-mode",
        choices=["left_moving_tracks", "wrist_track", "manip_track", "random_static_track"],
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="0=realsense_left, 1=surround_cam_moving for left_moving_tracks.",
    )
...
def build_eval_args(args: argparse.Namespace) -> argparse.Namespace:
    eval_parser = eval_scal3r.build_argparser()
    ...
    return eval_scal3r.args_from_run_json(eval_args)


def load_batch(args: argparse.Namespace) -> Dict[str, object]:
    eval_args = build_eval_args(args)
    loader, scenes = eval_scal3r.build_scal3r_native_eval_loader(eval_args, eval_mode=args.eval_mode)
    if len(loader) <= 0:
        raise RuntimeError("eval_scal3r loader produced no batches")
    ...
    for batch_idx, batch in enumerate(loader):
        if batch_idx == target:
            ...
            print(
                f"[batch] geometry=scal3r_native image_size={eval_args.image_size} "
                f"patch_size={eval_args.patch_size} preprocess_mode={eval_args.preprocess_mode}"
            )
            print(
...
def run_viser(args: argparse.Namespace, batch: Dict[str, object]) -> None:
    ...
    with server.gui.add_folder("eval_scal3r GT batch"):
