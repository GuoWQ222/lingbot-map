#!/usr/bin/env python3
"""Visualize one ScanNet++ trajectory sampled by the training DataLoader."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sample one ScanNet++ training trajectory and visualize its RGB-D point cloud."
    )
    parser.add_argument(
        "--root",
        default="/shared/smartbot/renkerui/data/scannetppv2",
        help="ScanNet++ v2 root. Defaults to SCANNETPP_ROOT in train.sh.",
    )
    ...
    parser.add_argument("--video-prob", type=float, default=0.6)
...
def make_dataloader(args: argparse.Namespace) -> Tuple[DataLoader, ScanNetppTrajectoryDataset, int]:
    # These defaults mirror train.sh and build_dataloaders() in train.py.
    dataset = ScanNetppTrajectoryDataset(
        root=args.root,
        split="train",
    ...
    if args.scene:
        if args.scene not in dataset.sequences:
            raise ValueError(
                f"Scene {args.scene!r} is not in the ScanNet++ training dataset "
...
def run_viser(args: argparse.Namespace, batch: Dict[str, object]) -> None:
    ...
    with server.gui.add_folder("ScanNet++ trajectory"):
