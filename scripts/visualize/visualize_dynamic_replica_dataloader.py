#!/usr/bin/env python3
"""Visualize one Dynamic Replica trajectory sampled by the training DataLoader."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sample one Dynamic Replica training trajectory and visualize its RGB-D point cloud."
    )
    parser.add_argument(
        "--root",
        default="/shared/smartbot/renkerui/data/dynamic_replica",
        help="Dynamic Replica root. Defaults to DYNAMIC_REPLICA_ROOT in train.sh.",
    )
    ...
    parser.add_argument("--video-prob", type=float, default=1.0)
    parser.add_argument("--fix-interval-prob", type=float, default=1.0)
...
def make_dataloader(args: argparse.Namespace) -> Tuple[DataLoader, DynamicReplicaTrajectoryDataset, int]:
    # These defaults mirror train.sh and build_dataloaders() in train.py.
    dataset = DynamicReplicaTrajectoryDataset(
        root=args.root,
        split="train",
    ...
    if args.scene:
        if args.scene not in dataset.sequences:
            raise ValueError(
                f"Scene {args.scene!r} is not in the Dynamic Replica training dataset "
...
def run_viser(args: argparse.Namespace, batch: Dict[str, object]) -> None:
    ...
    with server.gui.add_folder("Dynamic Replica trajectory"):
