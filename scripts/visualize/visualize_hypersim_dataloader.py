#!/usr/bin/env python3
"""Visualize one Hypersim trajectory sampled by the training DataLoader."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sample one Hypersim training trajectory and visualize its RGB-D point cloud."
    )
    parser.add_argument(
        "--root",
        default="/oss-guowenqi/hypersim",
        help="Hypersim root. Defaults to HYPERSIM_ROOT in train.sh.",
    )
...
def make_dataloader(args: argparse.Namespace) -> Tuple[DataLoader, HypersimTrajectoryDataset, int]:
    ...
    if args.scene:
        if args.scene not in dataset.sequences:
            raise ValueError(
                f"Scene {args.scene!r} is not in the Hypersim training dataset "
...
def run_viser(args: argparse.Namespace, batch: Dict[str, object]) -> None:
    ...
    with server.gui.add_folder("Hypersim trajectory"):
