#!/usr/bin/env python3
"""Visualize one ARKitScenes HighRes trajectory sampled by the training DataLoader."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sample one ARKitScenes HighRes training trajectory and visualize its RGB-D point cloud."
    )
    parser.add_argument(
        "--root",
        default="/shared/smartbot/renkerui/data/arkitscenes_highres",
        help="ARKitScenes HighRes root. Defaults to ARKITSCENES_HIGHRES_ROOT in train.sh.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Only filters short scenes; ARKitScenes sampling remains fixed at --num-views.",
    )
    ...
    parser.add_argument("--min-interval", type=int, default=1)
    parser.add_argument("--max-interval", type=int, default=32)
    parser.add_argument("--video-prob", type=float, default=0.5)
    parser.add_argument("--fix-interval-prob", type=float, default=0.5)
    parser.add_argument("--block-shuffle", type=int, default=16)
    parser.add_argument(
        "--training-transform",
        action="store_true",
...
def make_dataloader(args: argparse.Namespace) -> Tuple[DataLoader, ARKitScenesHighResTrajectoryDataset, int]:
    # These defaults mirror train.sh and build_dataloaders() in train.py.
    dataset = ARKitScenesHighResTrajectoryDataset(
        root=args.root,
        split="train",
        num_views=args.num_views,
        min_views=min(args.min_views, args.num_views),
        ...
        fix_interval_prob=args.fix_interval_prob,
    ...
    if args.scene:
        if args.scene not in dataset.scenes:
            raise ValueError(
                f"Scene {args.scene!r} is not in the ARKitScenes HighRes training dataset "
                f"({len(dataset.scenes)} scenes)."
            )
        scene_idx = dataset.scenes.index(args.scene)
        sample_index = next(
            idx
            for idx, (ref_scene_idx, _) in enumerate(dataset.start_refs)
            if ref_scene_idx == scene_idx
        )
    else:
        sample_index = args.index % len(dataset)
...
def run_viser(args: argparse.Namespace, batch: Dict[str, object]) -> None:
    ...
    with server.gui.add_folder("ARKitScenes HighRes trajectory"):
