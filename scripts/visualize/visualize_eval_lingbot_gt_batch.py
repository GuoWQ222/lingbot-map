#!/usr/bin/env python3
"""Visualize one LingBot-MAP validation batch as a GT RGB-D point cloud."""
...
DEFAULT_ARGS_JSON = (
    "/cpfs/user/guowenqi/lingbot-map-copy/outputs/runs/"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Load one eval batch from the run.sh validation split and visualize its GT point cloud."
    )
    parser.add_argument("--train-args-json", default=DEFAULT_ARGS_JSON)
    parser.add_argument("--split", choices=["val", "train", "all"], default="val")
    parser.add_argument(
        "--eval-mode",
        choices=["default", "left_moving_tracks", "wrist_track", "manip_track", "random_static_track"],
        default="default",
        help="default keeps one batch per validation trajectory; left_moving_tracks yields wrist + moving-surround clips.",
    )
    parser.add_argument("--max-scenes-eval", type=int, default=0)
...
def build_eval_args(args: argparse.Namespace) -> argparse.Namespace:
    args_json = Path(args.train_args_json)
    with args_json.open("r", encoding="utf-8") as f:
        data = json.load(f)
    ns = argparse.Namespace(**data)
    ns.train_args_json = str(args_json)
    ns.split = str(args.split)
    ns.max_scenes_eval = int(args.max_scenes_eval)
    ns.eval_shard_count = 1
    ns.eval_shard_index = 0
    ns.num_workers = int(args.num_workers)
    ns.device = "cpu"
    ns.eval_strategy = str(args.eval_mode)
    ns.eval_num_frames = int(args.eval_num_frames)
    ns.eval_wrist_camera_name = str(args.eval_wrist_camera_name)
    ns.eval_surround_camera_name = str(args.eval_surround_camera_name)
    ns.eval_seed = int(args.eval_seed)
    return ns
...
def load_batch(args: argparse.Namespace) -> Dict[str, object]:
    ...
    if len(loader) <= 0:
        raise RuntimeError("eval loader produced no batches")
    target = int(args.batch_index) % len(loader)
    for batch_idx, batch in enumerate(loader):
        ...
        print(
            f"[batch] split={args.split} val_scenes={len(scenes)} batches={len(loader)} "
            f"batch_index={target} scene={scene} sample_mode={sample_mode}"
        )
        print(
            f"[batch] image_size={eval_args.image_size} patch_size={eval_args.patch_size} "
            f"preprocess_mode={eval_args.preprocess_mode} eval_mode={args.eval_mode}"
        )
        print(
...
def run_viser(args: argparse.Namespace, batch: Dict[str, object]) -> None:
    ...
    with server.gui.add_folder("LingBot val GT batch"):
    ...
    for frame_idx, (pose, K, image, frame_id) in enumerate(zip(c2w, intrinsics, images, frame_ids)):
        ...
        fov = float(2.0 * np.arctan2(height / 2.0, max(float(K[1, 1]), 1e-6)))
