#!/usr/bin/env python3
"""Visualize one TartanAir trajectory sampled by the training DataLoader."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sample one TartanAir training trajectory and visualize its RGB-D point cloud."
    )
    parser.add_argument(
        "--root",
        default="/cpfs/shared/landmark/renkerui/data/tartanair",
        help="TartanAir root. Defaults to TARTANAIR_ROOT in train.sh.",
    )
    ...
    parser.add_argument("--video-prob", type=float, default=0.8)
    parser.add_argument("--fix-interval-prob", type=float, default=0.6)
...
def make_dataloader(args: argparse.Namespace) -> Tuple[DataLoader, TartanAirTrajectoryDataset, int]:
    # These defaults mirror train.sh and build_dataloaders() in train.py.
    dataset = TartanAirTrajectoryDataset(
        root=args.root,
        split="train",
    ...
    if args.scene:
        if args.scene not in dataset.sequences:
            raise ValueError(
                f"Scene {args.scene!r} is not in the TartanAir training dataset "
                f"({len(dataset.sequences)} scenes)."
            )
        sample_index = dataset.sequences.index(args.scene)
    else:
...
def run_viser(args: argparse.Namespace, batch: Dict[str, object]) -> None:
    ...
    with server.gui.add_folder("TartanAir trajectory"):
