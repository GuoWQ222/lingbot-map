#!/usr/bin/env python3
"""Visualize one Manip_long6 surround_cam_fixed trajectory sampled by the training DataLoader."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sample one Manip_long6 surround_cam_fixed training trajectory and visualize "
    ...
    parser.add_argument(
        ...
        default=False,
        help="Use exported masks if present. Default is false because Manip_long6 surround_cam_fixed exports do not include masks.",
    )

    parser.add_argument(
        "--camera-names",
        default="surround_cam_fixed",
        help="Comma-separated Manip_long6 camera dirs to allow.",
    )
    parser.add_argument("--fixed-stride-min", type=int, default=4)
    parser.add_argument("--fixed-stride-max", type=int, default=16)
...
def make_dataloader(args: argparse.Namespace) -> Tuple[DataLoader, ManipTrajectoryDataset, int, List[Path]]:
    ...
    dataset = ManipTrajectoryDataset(
        ...
        long6_mode_weights={"W": 0.0, "T": 0.0, "F": 1.0, "M": 0.0},
    ...
    if args.scene:
        scene_names = [scene.name for scene in train_scenes]
        scene_paths = [str(scene) for scene in train_scenes]
        if args.scene in scene_names:
            sample_index = scene_names.index(args.scene)
        elif args.scene in scene_paths:
            sample_index = scene_paths.index(args.scene)
        else:
            raise ValueError(
...
def run_viser(args: argparse.Namespace, batch: Dict[str, object]) -> None:
    ...
    with server.gui.add_folder("Manip_long6 surround_cam_fixed trajectory"):
