...
[eval_scal3r]
  ...
  pointcloud    : ${POINTCLOUD_METRICS} source=depth+c2w align=${POINTCLOUD_ALIGN} backend=${POINTCLOUD_ICP_BACKEND} kdtree_workers=${POINTCLOUD_KDTREE_WORKERS} workers=${POINTCLOUD_WORKERS}
========================================

EOF

"${PYTHON_CMD[@]}" "${REPO_DIR}/scripts/eval/eval_scal3r.py" "${ARGS[@]}" "$@"
