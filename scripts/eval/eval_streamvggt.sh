...
export PYTHONPATH="${REPO_DIR}:${REPO_DIR}/scripts/train:${REPO_DIR}/scripts/eval:${STREAMVGGT_REPO}/src:${PYTHONPATH:-}"
...
RUN_DIR="${RUN_DIR:-${REPO_DIR}/outputs/runs/manip_long_train_64gpu}"
...
[eval_streamvggt]
  ...
  pointcloud    : ${POINTCLOUD_METRICS} source=streamvggt_depth+pose align=${POINTCLOUD_ALIGN} backend=${POINTCLOUD_ICP_BACKEND} kdtree_workers=${POINTCLOUD_KDTREE_WORKERS} workers=${POINTCLOUD_WORKERS}
========================================

EOF

"${PYTHON_CMD[@]}" "${REPO_DIR}/scripts/eval/eval_streamvggt.py" "${ARGS[@]}" "$@"
