...
# Fast checkpoint-ranking preset. It keeps the same val split and eval codepath
# as eval.sh, but evaluates one moving surround camera and uses fewer linspace
# frames per clip.
CHECKPOINT_DIR="${CHECKPOINT_DIR:-${REPO_DIR}/outputs/runs/manip_long_train_64gpu}"
CHECKPOINT_GLOB="${CHECKPOINT_GLOB:-checkpoint_*.pt}"
TRAIN_ARGS_JSON="${TRAIN_ARGS_JSON:-${CHECKPOINT_DIR}/args.json}"
EVAL_NUM_FRAMES="${EVAL_NUM_FRAMES:-32}"
EVAL_SURROUND_CAMERA_NAME="${EVAL_SURROUND_CAMERA_NAME:-surround_cam_moving}"
NUM_WORKERS="${NUM_WORKERS:-4}"
PER_SCENE_CSV="${PER_SCENE_CSV:-0}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${CHECKPOINT_DIR}/eval_fast_n${EVAL_NUM_FRAMES}_${EVAL_SURROUND_CAMERA_NAME}}"
...
if (( MAX_PARALLEL > ${#GPU_IDS[@]} )); then
  echo "[eval_fast_parallel.sh][warn] MAX_PARALLEL=${MAX_PARALLEL} exceeds GPU count=${#GPU_IDS[@]}; devices will be reused."
fi
...
if (( ${#CHECKPOINT_LIST[@]} == 0 )); then
  echo "[eval_fast_parallel.sh] no checkpoints matched" >&2
  exit 2
fi
...
for idx in "${!CHECKPOINT_LIST[@]}"; do
  ...
  if [[ ! -f "${ckpt}" ]]; then
    echo "[eval_fast_parallel.sh] checkpoint not found: ${ckpt}" >&2
    exit 2
  fi
  ...
  if [[ "${SKIP_EXISTING}" == "1" && -f "${out_dir}/metrics.json" ]]; then
    echo "[eval_fast_parallel.sh] skip existing ${name}"
    continue
  fi

  gpu="${GPU_IDS[$(( idx % ${#GPU_IDS[@]} ))]}"
  cat <<EOF
[eval_fast_parallel.sh] launch ${name}
  gpu        : ${gpu}
  checkpoint : ${ckpt}
  output     : ${out_dir}
  log        : ${log_path}
...
base_fields = ["checkpoint", "checkpoint_name", "step", "split", "eval_strategy", "eval_num_frames"]
rows = []
...
for metrics_path in metric_paths:
    if not metrics_path.is_file():
        print(f"[eval_fast_parallel.sh][warn] missing metrics: {metrics_path}")
        continue
...
print(f"[eval_fast_parallel.sh] wrote {summary_csv} ({len(rows)} rows)")
PY_SUMMARY

echo "[eval_fast_parallel.sh] done"
