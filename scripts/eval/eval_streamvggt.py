...
DEFAULT_STREAMVGGT_REPO = REPO_DIR.parent / "StreamVGGT"
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--streamvggt_preprocess_mode", choices=["crop", "pad"], default="crop",
                   help="StreamVGGT-native image geometry; depth/mask/intrinsics/world_points use LingBot-MAP sync.")
...
def args_from_run_json(eval_args: argparse.Namespace) -> argparse.Namespace:
    ...
    ns.patch_size = 14
    ns.streamvggt_preprocess_mode = str(getattr(eval_args, "streamvggt_preprocess_mode", "crop"))
    ns.vggt_preprocess_mode = ns.streamvggt_preprocess_mode
    ns.preprocess_mode = ns.streamvggt_preprocess_mode
...
def evaluate_one_mode(
    ...
    print(f"[eval_streamvggt] split={args.split}, scenes={len(eval_scenes)}, batches={len(loader)}, "
          ...
          f"{getattr(args, 'streamvggt_preprocess_mode', getattr(args, 'preprocess_mode', 'unknown'))}, "
    ...
    summary: Dict[str, object] = {
        ...
        "streamvggt_preprocess_mode": str(getattr(args, "streamvggt_preprocess_mode", getattr(args, "preprocess_mode", "crop"))),
