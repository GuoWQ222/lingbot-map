...
export PYTHONPATH="${REPO_DIR}:${REPO_DIR}/scripts/train:${REPO_DIR}/scripts/eval:${VGGT_REPO}:${PYTHONPATH:-}"
...
[eval_vggt]
  ...
  pointcloud    : ${POINTCLOUD_METRICS} source=depth+pose align=${POINTCLOUD_ALIGN} backend=${POINTCLOUD_ICP_BACKEND} kdtree_workers=${POINTCLOUD_KDTREE_WORKERS} workers=${POINTCLOUD_WORKERS}
========================================

EOF

"${PYTHON_CMD[@]}" "${REPO_DIR}/scripts/eval/eval_vggt.py" "${ARGS[@]}" "$@"
