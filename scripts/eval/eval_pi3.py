...
def compute_pi3_preprocess_geometry(width: int, height: int, new_width: int) -> Dict[str, int]:
    """Match Pi3 utils.interfaces.load_images(..., new_width=...) geometry."""
    if width <= 0 or height <= 0:
        raise ValueError(f"Invalid source image size: {width}x{height}")
    target_width = max(14, int(new_width))
    target_height = round(height * (target_width / width) / 14) * 14
    target_height = max(14, target_height)
    return {
...
def load_images_like_pi3(filelist: List[str], new_width: int = 518) -> torch.Tensor:
    """Dependency-light copy of Pi3 utils.interfaces.load_images for path lists."""
    sources = []
    ...
    geometry = compute_pi3_preprocess_geometry(width, height, int(new_width))
    target_size = (geometry["new_width"], geometry["new_height"])
    tensor_list = []
    for img_pil in sources:
        resized_img = img_pil.resize(target_size, Image.Resampling.LANCZOS)
        tensor_list.append(TF.to_tensor(resized_img))
    if not tensor_list:
        return torch.empty(0)
    return torch.stack(tensor_list, dim=0)


class Pi3NativeEvalDataset(LBE.EvalLinspaceDataset):  # type: ignore[misc]
    """Eval sampler with RGB-D and GT loaded onto Pi3's native image grid."""
    ...
    def _load_one(
        ...
        geometry = compute_pi3_preprocess_geometry(width, height, int(self.image_size))
    ...
    def __getitem__(self, index: int) -> Dict[str, object]:
        ...
        for attempt in range(min(16, len(self._eval_index_map))):
            ...
            try:
                ...
                last_error = exc
                continue
        raise RuntimeError(f"Failed to load a valid Pi3 native eval sample near index {index}: {last_error}")


def collate_pi3_native_sequences(samples: List[Dict[str, object]]) -> Dict[str, object]:
    batch = T.collate_rgbd_sequences(samples)
    batch["rgb_paths"] = [list(cast(List[str], sample["rgb_paths"])) for sample in samples]
    return batch




class Pi3NativeEvalDataset(EMG.ModelGeometryEvalDataset):  # type: ignore[misc]
    """Eval sampler whose GT tensors are synchronized to Pi3 native geometry."""

    rgb_resample = Image.Resampling.LANCZOS

    def model_preprocess_geometry(self, width: int, height: int) -> Dict[str, int]:
        return compute_pi3_preprocess_geometry(width, height, int(self.image_size))
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--output_dir", type=str, default=str(REPO_DIR / "outputs" / "eval" / "pi3"))
    ...
    p.add_argument("--image_size", type=int, default=0,
                   help="Override LingBot image_size from train_args_json. Used only with --pi3_input_mode lingbot.")
    p.add_argument("--pi3_input_mode", choices=["native", "lingbot"], default="native",
                   help="native uses Pi3 utils.interfaces.load_images from rgb paths and loads GT on the same PI3 grid; "
                        "lingbot keeps the old LingBot-MAP preprocessed images.")
    p.add_argument("--pi3_native_width", type=int, default=518,
                   help="new_width passed to Pi3 utils.interfaces.load_images for native input mode.")
    ...
    p.add_argument("--recover_focal", action=argparse.BooleanOptionalAction, default=True,
                   help="Recover intrinsics from Pi3 local point maps when needed for unprojection.")
...
def coerce_pi3_args_from_json(eval_args: argparse.Namespace) -> argparse.Namespace:
    ...
    ns.pi3_input_mode = str(eval_args.pi3_input_mode)
    ns.pi3_native_width = int(eval_args.pi3_native_width)
...
def build_pi3_native_eval_loader(args: argparse.Namespace, eval_mode: str = "default") -> Tuple[DataLoader, List[Path]]:
    ...
    if shard_count > 1:
        ...
        print(f"[eval_pi3] shard {shard_index}/{shard_count}: {len(eval_scenes)} of {total_eval_scenes} scenes")
    if not eval_scenes:
    ...
    common_kwargs = dict(
        ...
        image_size=int(getattr(args, "pi3_native_width", 518)),
        patch_size=14,
    ...
    if eval_mode == "default":
        raise ValueError("Pi3 native input mode requires a deterministic eval track mode, not train_default")
    if eval_mode not in {"left_moving_tracks", "manip_track", "wrist_track", "random_static_track"}:
        raise ValueError(f"Unknown eval_mode: {eval_mode}")
    dataset = Pi3NativeEvalDataset(
        eval_scenes,
        eval_mode=eval_mode,
    ...
    loader_kwargs = dict(
        ...
        drop_last=False,
        collate_fn=collate_pi3_native_sequences,
    )
    if nw > 0:
...
def build_pi3_eval_loader(args: argparse.Namespace, eval_mode: str = "default") -> Tuple[DataLoader, List[Path]]:
    if str(getattr(args, "pi3_input_mode", "native")) == "native":
        return build_pi3_native_eval_loader(args, eval_mode=eval_mode)
    return LBE.build_eval_loader(args, eval_mode=eval_mode)


def load_pi3_native_images_from_paths(args: argparse.Namespace, batch: Dict[str, object], device: torch.device) -> torch.Tensor:
    rgb_paths_batch = batch.get("rgb_paths")
    if not rgb_paths_batch:
        raise ValueError("Pi3 native input mode requires rgb_paths in the eval batch")
    if len(cast(List[object], rgb_paths_batch)) != 1:
        raise ValueError("Pi3 eval expects batch_size=1 for native image loading")
    rgb_paths = [str(path) for path in cast(List[List[str]], rgb_paths_batch)[0]]

    pi3_repo = Path(args.pi3_repo).resolve()
    if str(pi3_repo) not in sys.path:
        sys.path.insert(0, str(pi3_repo))
    try:
        from utils.interfaces import load_images
    except ModuleNotFoundError as exc:
        if exc.name not in {"omegaconf", "rootutils"}:
            raise
        load_images = None

    native_width = int(getattr(args, "pi3_native_width", 518))
    if load_images is None:
        images = load_images_like_pi3(rgb_paths, new_width=native_width)
    else:
        images = load_images(
            rgb_paths,
    ...
    if images.numel() == 0:
        raise RuntimeError("Pi3 native image loader returned an empty tensor")
    return images.unsqueeze(0).to(device, non_blocking=True)
...
def evaluate(args: argparse.Namespace) -> Dict[str, object]:
    ...
    modes = {
        ...
        "left_moving_tracks": ["left_moving_tracks"],
...
def _evaluate_one_mode(
    ...
    loader, eval_scenes = build_pi3_eval_loader(args, eval_mode=eval_mode)
    model_width = int(getattr(args, "pi3_native_width", 518)) if str(getattr(args, "pi3_input_mode", "native")) == "native" else int(args.image_size)
    print(f"[eval_pi3] split={args.split}, scenes={len(eval_scenes)}, batches={len(loader)}, "
          ...
          f"input_mode={getattr(args, 'pi3_input_mode', 'native')}, model_width={model_width}")
    ...
    all_depth_pixels = LBE.DepthPixelAccumulator()
    ...
    group_depth_pixels: Dict[str, LBE.DepthPixelAccumulator] = {
        name: LBE.DepthPixelAccumulator() for name in report_group_names
    }
    ...
    pointcloud_queue = LBE.AsyncPointcloudMetricQueue(
        args,
        all_pointcloud_rows,
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        if str(getattr(args, "pi3_input_mode", "native")) == "native":
            images_for_model = load_pi3_native_images_from_paths(args, batch, device)
            batch = T.to_device(batch, device)
            batch["images"] = images_for_model
        else:
            batch = T.to_device(batch, device)
        ...
        if metric_group not in group_counts:
            ...
            group_depth_pixels[metric_group] = LBE.DepthPixelAccumulator()
        ...
        clip_depth = LBE.DepthPixelAccumulator()
        clip_depth.update(pred_depth, gt_depth, mask, align=str(args.depth_align))
        ...
        for k, v in clip_depth.summary().items():
            row[f"depth_{k}"] = v
    ...
    summary: Dict[str, object] = {
        ...
        "camera_align": str(getattr(args, "camera_align", "none")),
        "pointcloud_align": str(getattr(args, "pointcloud_align", "none")),
        "pi3_input_mode": str(getattr(args, "pi3_input_mode", "native")),
        "pi3_native_width": int(getattr(args, "pi3_native_width", 518)),
        "input_geometry_sync": "model_native_geometry+lingbot_map_sync" if str(getattr(args, "pi3_input_mode", "native")) == "native" else "lingbot_map",
    ...
    overall_summary = LBE.summarize_metric_group(
    ...
    for group_name in report_group_names:
        if group_counts.get(group_name, 0) <= 0:
            continue
        groups[group_name] = LBE.summarize_metric_group(
