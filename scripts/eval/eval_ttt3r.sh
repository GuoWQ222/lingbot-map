...
export PYTHONPATH="${REPO_DIR}:${REPO_DIR}/scripts/train:${REPO_DIR}/scripts/eval:${TTT3R_REPO}:${TTT3R_REPO}/src:${PYTHONPATH:-}"
...
[eval_ttt3r]
  ...
  pointcloud    : ${POINTCLOUD_METRICS} source=pts3d+c2w align=${POINTCLOUD_ALIGN} backend=${POINTCLOUD_ICP_BACKEND} kdtree_workers=${POINTCLOUD_KDTREE_WORKERS} workers=${POINTCLOUD_WORKERS}
========================================

EOF

"${PYTHON_CMD[@]}" "${REPO_DIR}/scripts/eval/eval_ttt3r.py" "${ARGS[@]}" "$@"
