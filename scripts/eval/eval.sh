...
[eval]
  ...
  pointcloud   : ${POINTCLOUD_METRICS} max_points=${POINTCLOUD_MAX_POINTS} align=${POINTCLOUD_ALIGN} icp_threshold=${POINTCLOUD_ICP_THRESHOLD} icp_iters=${POINTCLOUD_ICP_MAX_ITERATIONS} workers=${POINTCLOUD_WORKERS} kdtree_workers=${POINTCLOUD_KDTREE_WORKERS}
========================================
...
EOF
  ...
  "${PYTHON_CMD[@]}" "${REPO_DIR}/scripts/eval/eval.py" "${ARGS[@]}" "$@" &
  EVAL_PID=$!
  wait "${EVAL_PID}"
  EXIT_CODE=$?
  EVAL_PID=""
