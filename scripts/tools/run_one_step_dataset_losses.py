...
for path in (REPO_DIR, REPO_DIR / "scripts" / "train", SCRIPT_DIR):
...
def make_args():
    ...
    args.model_path = str(REPO_DIR / "checkpoints" / "lingbot-map.pt")
    args.output_dir = str(REPO_DIR / "outputs" / "runs" / "one_step_dataset_losses_20260527")
    args.max_steps = 1
    args.batch_size = 1
    args.num_workers = 0
...
def main() -> None:
    ...
    out_path = Path(os.environ.get(
        "ONE_STEP_LOSS_JSONL",
        str(REPO_DIR / "outputs" / "runs" / "one_step_dataset_losses_20260527" / "results.jsonl"),
    ))
