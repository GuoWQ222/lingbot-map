...
a piecewise-linear schedule, then for each of ``epoch_length`` samples it:
  ...
  2. If External, picks one enabled external dataset by relative weights.
...
Per-external probability is ``(1 - p_manip) * normalized_external_weight``.
...
class CurriculumMixtureSampler(Sampler[int]):
    ...
    external_weights:
        Optional relative weights for enabled external datasets. Can be a
        sequence aligned with ``external_names`` or a mapping keyed by name.
        Missing mapping entries default to 1.0.
    epoch_length:
    ...
    def __init__(
        ...
        *,
        ...
        epoch_length: int,
        external_weights: Optional[Union[Sequence[float], Mapping[str, float]]] = None,
        p_manip_start: float = 0.60,
        p_manip_end: float = 0.80,
        warmup_start: int = 2000,
        warmup_end: int = 25000,
        seed: int = 0,
    ) -> None:
        if manip_size <= 0:
        ...
        self.external_weights = self._normalize_external_weights(external_weights)
    ...
    def _normalize_external_weights(
        self,
        external_weights: Optional[Union[Sequence[float], Mapping[str, float]]],
    ) -> List[float]:
        if external_weights is None:
            weights = [1.0 for _ in self.external_names]
        elif isinstance(external_weights, Mapping):
            weights = [float(external_weights.get(name, 1.0)) for name in self.external_names]
        else:
            if len(external_weights) != len(self.external_names):
                raise ValueError(
                    f"external_weights (len={len(external_weights)}) and external_names "
                    f"(len={len(self.external_names)}) must have the same length"
                )
            weights = [float(w) for w in external_weights]

        for i, w in enumerate(weights):
            if w < 0:
                raise ValueError(
                    f"external_weights[{i}] ({self.external_names[i]}) must be >= 0, got {w}"
                )
        total = sum(weights)
        if total <= 0:
            raise ValueError("external_weights must contain at least one positive value")
    ...
    def get_dataset_weights(self, step: Optional[int] = None) -> Dict[str, float]:
        ...
        if n_ext == 0:
            ...
            return out
        p_ext_total = 1.0 - p_m
        for name, weight in zip(self.external_names, self.external_weights):
            out[name] = p_ext_total * weight
        return out
    ...
    def __iter__(self):
        ...
        for _ in range(self.epoch_length):
            if n_ext == 0 or rng.random() < p_manip:
                ...
                ds = int(rng.choice(n_ext, p=self.external_weights))
